...
const Navbar = () => {
    ...
    return (
        <nav className="min-w-full flex flex-wrap items-center justify-between px-6 py-8 shadow-lg shadow-[#CBD5E180] bg-[#f6f6ef] relative z-10 font-['BrownStd'] font-[400]">
            ...
            <div className="hidden md:flex flex-1 justify-start items-center gap-4 md:gap-6">
                {[{ name: "Home", path: "/" }, { name: "About", path: "/about" }, { name: "Proposal", path: "/proposal" }].map((link) => (
            ...
            <div className="flex-shrink-0 cursor-pointer md:absolute md:left-1/2 md:transform md:-translate-x-1/2 md:flex">
                <img src="./malibu_tides_.png" onClick={handleNavigateOnLogoClick} className="h-10" alt="Logo" />
            </div>
            ...
            <div className="hidden md:flex flex-1 justify-end items-center gap-4 md:gap-6">
                ...
                <button onClick={handleNavigate} className="bg-[#cdcbb6] text-sm px-5 py-2 rounded-[19px] font-medium">Get Involved</button>
            </div>
